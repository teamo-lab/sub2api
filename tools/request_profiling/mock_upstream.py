...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_POST(self):
  ...
  slow='LOCAL_PROFILE_SLOW' in json.dumps(payload.get('input',''))
  if slow:events.insert(1,{'type':'response.reasoning_summary_text.delta','delta':'local thinking only'})
  try:
   for e in events:
    if slow and e['type']=='response.output_text.delta':time.sleep(2.5)
    time.sleep(.15);self.wfile.write(('event: '+e['type']+'\ndata: '+json.dumps(e)+'\n\n').encode());self.wfile.flush()
