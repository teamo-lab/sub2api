...
- `gateway.request_profiling_group_ids` 可限定分组；鉴权确定非目标分组后立即停止捕获，保留普通访问日志原行为。生产发布入口要求开启时给出非空分组列表。
- profile 以有界编码后的事件排队，单条128 KiB、队列/在途合计32 MiB；超限拒收计入普通丢弃和 profile 拒收计数。普通错误日志不受 profile 字节额度限制。批量解码与入库仍有短暂额外分配。
- `gateway.request_profiling_retention_hours` 默认6；既有清理服务每5分钟以3秒/5000行上限移除过期实测 profile 字段，跳过忙行并使用事务 advisory lock。原访问日志与历史重建不删除；积压/失败不会伪装成已经清理。
- scope、保留时间、profile 字节水位与拒收计数在页面公开显示。采集仍默认关闭。
