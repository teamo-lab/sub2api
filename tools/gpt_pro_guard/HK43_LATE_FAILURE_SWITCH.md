...
本候选默认关闭，尚未部署或启用。应用层不硬编码机器的数据库账号/分组 ID；初次生产实验仅限真正 43 (`ins-4r9sd5og` / `43.154.93.133`) 的 **#22、认证 API Key group 3**，唯一实验范围见 `hk43_late_failure_switch.experiment.json`。该文件只是准备清单，不执行模型请求或配置写入。
