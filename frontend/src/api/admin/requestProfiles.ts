...
export interface ProfileResult {recording_enabled?:boolean;rows:ProfileRow[];summary:{count:number;mean_us:number;p90_us:number;truncated:number;retries:number;fallbacks:number;local_reselect:number;stages:{name:string;mean_us:number}[]};page:number;limit:number;health?:{dropped_count:number;write_failed_count:number;queue_depth:number}}
