<template>
  <div class="pt-6">
    <div class="relative h-10 w-full rounded-lg bg-slate-100 dark:bg-slate-800" role="group" :aria-label="label">
      ...
      <span v-for="(e,i) in highlights" :key="`event-${i}`" class="absolute -top-5 z-20 -translate-x-1/2 cursor-help text-lg font-bold" :style="{left:`${Math.min(99,e.at_us/scale*100)}%`,color:e.kind==='fallback'?'#e11d48':'#d97706'}" :title="`${eventNames[e.kind]} · ${duration(e.at_us)} · 账号 #${e.account_id}`">◆</span>
    </div>
...
const highlights=computed(()=>props.events.filter(e=>e.kind==='retry'||e.kind==='fallback'))
</script>
