...
func Finish(ctx context.Context, at time.Time) *Snapshot {
	...
	return &Snapshot{ConcurrentUpstreams: r.parallel, DeliveryObservationSupported: r.deliverySupported, DownstreamFirstWriteUS: copyTime(r.firstWrite), DownstreamLastWriteUS: copyTime(r.lastWrite), DownstreamWriteErrorUS: copyTime(r.writeError), DownstreamBytes: r.writtenBytes, ClientDisconnectedUS: copyTime(r.disconnected), DownstreamFirstOutputUS: copyTime(r.deliveredOutput), DownstreamCompleteUS: copyTime(r.deliveredComplete), DownstreamErrorUS: copyTime(r.deliveredError), ClientOutcome: outcome, DrainAfterDisconnectUS: drain, Evidence: "measured", Version: 1, TotalUS: total, Spans: spans, Events: events, Segments: splitResponseBody(Partition(spans, total), r.protocol, r.parallel, r.deliverySupported, r.deliveredOutput), Dropped: r.dropped, Attempts: r.attempts, GroupID: r.group, Model: r.model, Protocol: r.protocol, BodyBytes: r.body}
}
