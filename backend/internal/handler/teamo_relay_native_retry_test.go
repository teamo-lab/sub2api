...
func newNativeRelayRetryHandler(t *testing.T, upstream service.HTTPUpstream, usageRepo service.UsageLogRepository) *OpenAIGatewayHandler {
	t.Helper()
...
func TestNativeRelayHTTPRetryAfterReasoningPrefix(t *testing.T) {
	...
	for _, ending := range []string{"tcp_reset", "short_chunk", "eof", "failed"} {
		t.Run(ending, func(t *testing.T) {
			...
			h := newNativeRelayRetryHandler(t, upstream, usageRepo)
			...
			if ending == "tcp_reset" || ending == "short_chunk" {
				select {
