	...
	return func(c *gin.Context) {
		...
		if status < 400 {
			if parsed.StreamFailure {
				...
				if final, ok := service.GetOpsStreamError(c); ok &&
					(final.LocalCapacityFailure || final.ClientBusinessLimited) && final.IntendedStatus >= 400 {
					status = final.IntendedStatus
				}
			} else {
		...
		// A captured SSE frame can also be generated by local admission. Only
		// hydrated attempt provenance decides responsibility, not inferred status.
		classifyHydratedOpsError(c, entry, parsed.Code, status, nil)
...
func logOpsStreamErrorValue(c *gin.Context, ops *service.OpsService, wireStatus int, streamErr service.OpsStreamError) {
	...
	if streamErr.Turn > 0 {
		...
		classifyHydratedOpsError(c, entry, streamErr.Code, classifyStatus, &streamErr)
	} else {
		classifyHydratedOpsError(c, entry, streamErr.Code, classifyStatus, nil)
	}
...
// Use the final-attempt fields that will actually be persisted. WS turns also
// own their client-admission marker, independent of later connection state.
func classifyHydratedOpsError(c *gin.Context, entry *service.OpsInsertErrorLogInput, code string, status int, snapshot *service.OpsStreamError) {
	origin := &gin.Context{}
	if c != nil {
		origin = c.Copy()
	}
	...
	origin.Set(service.OpsUpstreamStatusCodeKey, upstreamStatus)
	origin.Set(service.OpsUpstreamErrorsKey, entry.UpstreamErrors)
	if snapshot != nil {
		origin.Set(service.OpsClientBusinessLimitedKey, snapshot.ClientBusinessLimited)
		origin.Set(service.OpsClientBusinessLimitedReasonKey, snapshot.ClientBusinessLimitedReason)
		origin.Set(opsRoutingCapacityLimitedKey, false)
		origin.Set(service.OpsLocalCapacityFailureKey, snapshot.LocalCapacityFailure)
	}
	if origin.GetBool(service.OpsLocalCapacityFailureKey) {
		// Keep previous attempts in UpstreamErrors, but do not pair account B's
		// local rejection with account A's upstream status/message.
		entry.UpstreamStatusCode = nil
		entry.UpstreamErrorMessage = nil
		entry.UpstreamErrorDetail = nil
		entry.UpstreamModel = ""
	...
	entry.ErrorPhase, entry.IsBusinessLimited, entry.ErrorOwner, entry.ErrorSource = classifyOpsErrorLog(origin, entry.ErrorType, entry.ErrorMessage, code, status)
}
...
func shouldSkipFinalOpsFailure(c *gin.Context) bool {
	...
	if c.GetBool(service.OpsLocalCapacityFailureKey) {
		return c.GetBool(service.OpsSkipPassthroughKey)
	}
...
func classifyOpsErrorLog(c *gin.Context, errType, message, code string, status int) (phase string, isBusinessLimited bool, errorOwner string, errorSource string) {
	if c != nil && c.GetBool(service.OpsLocalCapacityFailureKey) {
	...
	if clientBusinessLimited && !upstreamError && !routingCapacityLimited && !localModelConfiguration {
		...
		case service.OpsClientBusinessLimitedReasonUserConcurrency, service.OpsClientBusinessLimitedReasonUserWaitQueue:
			phase = "request"
		}
	}
	if routingCapacityLimited && !upstreamError && !accountAuthFailure {
		phase = "routing"
	}
	msg := strings.ToLower(message)
