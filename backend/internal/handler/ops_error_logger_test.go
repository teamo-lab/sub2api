...
func TestOpsErrorLoggerMiddleware_FinalStreamFailureWinsOverRecoveredContext(t *testing.T) {
	setupOpsErrorLogTestQueue(t, 4)
	gin.SetMode(gin.TestMode)
	...
	router.POST("/v1/responses", func(c *gin.Context) {
		c.Set(opsModelKey, "gpt-5.6-sol")
		c.Set(service.OpsUpstreamErrorsKey, []*service.OpsUpstreamErrorEvent{{
			Kind: "recovered", Stage: "stream", Reason: "retry", Message: "first attempt failed",
		}})
		service.MarkOpsStreamFailure(c, "upstream_error", "response_failed", "final attempt failed", http.StatusBadGateway)
		c.Status(http.StatusOK)
	})
	...
	job := <-opsErrorLogQueue
	require.Equal(t, http.StatusBadGateway, job.entry.StatusCode)
	require.Equal(t, "final attempt failed", job.entry.ErrorMessage)
	require.Equal(t, "upstream_error", job.entry.ErrorType)
}
