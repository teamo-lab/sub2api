...
type ErrorPassthroughRule struct {
	RecoveryPolicy  *ErrorRecoveryPolicy `json:"recovery_policy,omitempty"`
	ID              int64                `json:"id"`
...
func (r *ErrorPassthroughRule) Validate() error {
	if r.RecoveryPolicy != nil {
		if err := r.RecoveryPolicy.Validate(); err != nil {
			return err
		}
		if r.RecoveryPolicy.Mode != "default" {
			if len(r.Platforms) != 1 || r.Platforms[0] != PlatformOpenAI {
				return &ValidationError{Field: "platforms", Message: "recovery policies currently require the OpenAI platform"}
			}
			if r.SkipMonitoring {
				return &ValidationError{Field: "skip_monitoring", Message: "recovery policies must retain monitoring"}
			}
		}
	}
	if r.Name == "" {
